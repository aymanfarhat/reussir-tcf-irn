  ...
  <article class="tcf-card">
    <div class="flex flex-wrap items-start justify-between gap-3">
      ...
      <div class="flex flex-wrap items-center justify-end gap-3">
        ...
        <UiBadge v-else tone="warning">{{ gradeStatusLabel(attempt.grade?.status || 'pending') }}</UiBadge>
        <UiButton class="shrink-0" variant="ghost" :to="`/tests/${sessionUuid}/tasks/${attempt.sequenceOrder}/grade`">{{ $t('report.viewTaskFeedback') }}</UiButton>
      </div>
    </div>
    ...
    <div v-if="attempt.grade?.status === 'succeeded'" class="mt-4 grid gap-4 md:grid-cols-2">
      <div>
        <h3 class="text-sm font-semibold text-slate-950">{{ $t('report.covered') }}</h3>
      ...
      <div>
        <h3 class="text-sm font-semibold text-slate-950">{{ $t('report.missing') }}</h3>
        <ul class="mt-2 list-disc space-y-1 pl-5 text-sm text-slate-600">
...
const { gradeStatusLabel, taskLabel } = useUiLabels()
</script>
