<template>
  <div>
    <div class="flex flex-wrap items-start justify-between gap-4">
      <div>
        <UiBadge :tone="attempt.isWritten ? 'written' : 'oral'">{{ taskLabel(attempt.question.taskNumber) }}</UiBadge>
      ...
      <UiBadge v-else :tone="grade?.status === 'failed' ? 'error' : 'warning'">{{ grade ? gradeStatusLabel(grade.status) : statusLabel(attempt.status) }}</UiBadge>
    </div>

    <div class="mt-6 rounded-md border border-slate-200 bg-slate-50 p-4">
      <h2 class="font-semibold text-slate-950">{{ $t('feedback.yourResponse') }}</h2>
      <p class="mt-3 whitespace-pre-wrap text-sm leading-6 text-slate-700">{{ attempt.candidateText || $t('feedback.noCandidate') }}</p>
    </div>

    <div v-if="grade?.status === 'succeeded'" class="mt-6 space-y-5">
      <div class="grid gap-4 md:grid-cols-2">
        <div class="rounded-md border border-lime-200 bg-lime-50 p-4">
          <h2 class="font-semibold text-lime-950">{{ $t('feedback.strengths') }}</h2>
        ...
        <div class="rounded-md border border-amber-200 bg-amber-50 p-4">
          <h2 class="font-semibold text-amber-950">{{ $t('feedback.improveNext') }}</h2>
      ...
      <div class="overflow-hidden rounded-md border border-slate-200">
        <table class="w-full text-left text-sm">
          <thead class="bg-slate-50 text-slate-600">
            <tr><th class="px-3 py-2">{{ $t('common.dimension') }}</th><th class="px-3 py-2">{{ $t('common.score') }}</th><th class="px-3 py-2">{{ $t('common.level') }}</th><th class="px-3 py-2">{{ $t('feedback.justification') }}</th></tr>
          </thead>
      ...
      <div v-if="grade.audioFeedback && Object.keys(grade.audioFeedback).length" class="rounded-md border border-indigo-200 bg-indigo-50 p-4">
        <div class="flex flex-wrap items-center justify-between gap-2">
          <h2 class="font-semibold text-indigo-950">{{ $t('feedback.audioDelivery') }}</h2>
          <span class="text-xs font-medium text-indigo-700">{{ grade.audioFeedback.audioEvidenceUsed || grade.audioFeedback.audio_evidence_used ? $t('feedback.audioEvidenceUsed') : $t('feedback.transcriptOnly') }}</span>
        </div>
        <div class="mt-3 grid gap-3 md:grid-cols-2">
          <div class="rounded-md bg-white p-3">
            <div class="text-xs font-medium uppercase text-slate-500">{{ $t('feedback.pronunciation') }}</div>
          ...
          <div class="rounded-md bg-white p-3">
            <div class="text-xs font-medium uppercase text-slate-500">{{ $t('feedback.fluency') }}</div>
...
const { gradeStatusLabel, statusLabel, taskLabel } = useUiLabels()
</script>
